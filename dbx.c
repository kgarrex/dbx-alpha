...
int dbx_command_error(dbcommand cmd, char *funcname)
{
#ifdef __ODBC__
  SQLRETURN rtn;
  char sqlstate[6];
...
int dbx_connect(dbconn conn)
{
  ...
  len = make_odbc_connection_string(buf, 256, &coninfo);
...
void dbx_disconnect(dbconn conn)
{
#ifdef __ODBC__
...
void dbx_bindparams(dbcommand cmd)
{
  int num_param = 0;
...
int dbx_connection_setprop(dbconn conn, enum db_connection_property prop, ...)
{
  va_list args;
  void *ptr;
  int len;
...
int dbx_command_setprop(dbcommand cmd, enum db_command_property prop, ...)
{
  va_list args;
  void *ptr;
  int n;
