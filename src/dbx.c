...
(char buf[256], int bufsize, struct odbc_conn_info *info)
{
  ...
  if(info->driver == OdbcDriver_SqlServer)
  {
  ...
  else if(info->driver == OdbcDriver_MSAccess)
  {
...
int dbx_command_error(dbx_command_t cmd, char *funcname)
{
#ifdef __ODBC__
  SQLRETURN rtn;
  char sqlstate[6];
...
void dbx_bindparams(dbx_command_t cmd)
{
  int num_param = 0;
...
int dbx_connection_setprop(dbx_conn_t conn, enum dbx_connection_property prop, ...)
{
  va_list args;
  void *ptr;
  int len;
...
#if defined(__ODBC__)
	case DbxConnProp_OdbcDriver: {
      int driver;
      driver = va_arg(args, int);
	  conn->odbc.info.driver = driver;
    }
    break;
#endif
...
int dbx_command_setprop(dbx_command_t cmd, enum dbx_command_property prop, ...)
{
  va_list args;
  void *ptr;
  int n;
...
int dbx_prepare(dbx_command_t cmd)
{
  if (!cmd->query_string) {
    return 0;
  }
...
int dbx_connection_new(dbx_conn_t *pconn)
{
#ifdef __ODBC__
  SQLRETURN rtn;
  if (!odbc_henv) {
  ...
  *pconn = malloc(sizeof(struct _dbx_conn));
...
int dbx_command_new(dbx_conn_t conn, dbx_command_t *pcmd)
{
  *pcmd = malloc(sizeof(struct _dbx_command));
  if (!*pcmd) {
    printf("Error: No memory\n");
    return 0;
  }
  ...
  if (rtn != SQL_SUCCESS) {
    dbx_command_error((*pcmd), "SQLSetStmtAttr scroll\n");
    return 0;
  }
...
int dbx_getprop(dbx_command_t cmd, int prop, ...)
{
  return 0;
}
...
void compare_column(dbx_command_t cmd, struct dbx_field_desc *fd, int col)
{
#if defined(__ODBC__)
  SQLRETURN rtn;
...
int _bind_columns(dbx_command_t cmd)
{
...
int _fetch_rows(dbx_command_t cmd)
{
  struct dbx_query_desc *qry_desc = cmd->query_desc;
